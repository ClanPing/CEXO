...
Process:
1. Generate an unbiased training pool of construction layouts
2. Train an autoencoder to learn two latent behavioral descriptors
3. Build a MAP-Elites archive with a bounded Pareto front in each cell
4. Continue genetic variation with mutation, crossover, and repair operators
5. Export CEXO layouts, metrics, and visualizations
"""
...
# Ensure local package imports resolve when the script is launched from anywhere.
...
from core.cexo_algorithm import CEXOOptimizer
...
def visualize_diverse_layouts(archive, algorithm, num_layouts=9):
    ...
    Args:
        ...
        algorithm: CEXOOptimizer instance
    ...
    site_config = algorithm.site_config
...
def run_mapelites_with_learned_bds(
    ...
    output_dir: str = "output/cexo",
    seed: int = 42,
    use_bulleen_boundary: bool = False,
    use_bulleen_roads: bool = False,
    ...
    Args:
        facility_count: Number of facilities in layout, ignored if facility_types is provided
        facility_types: Optional fixed facility list for a practical case study
    ...
    site_config = SiteConfig(
        ...
        boundary_polygon=get_bulleen_boundary_polygon() if use_bulleen_boundary else None,
        exclusion_zones=get_bulleen_road_polygons() if use_bulleen_roads else None,
        fixed_entrances=get_bulleen_entrances() if use_bulleen_entrances else None,
        seed=seed
    )
    ...
    print(f"  Site boundary: {'Bulleen irregular polygon' if site_config.boundary_polygon else 'unit square'}")
    if site_config.fixed_entrances:
        print(f"  Fixed entrances: {len(site_config.fixed_entrances)} Bulleen access points")
    if site_config.exclusion_zones:
        print(f"  Exclusion zones: {len(site_config.exclusion_zones)} road/access corridors")
    ...
    # Run CEXO: MAP-Elites archive + per-cell Pareto selection + optional autoencoder BDs.
    algorithm = CEXOOptimizer(
        facility_types=facility_types,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--initial-pop', '--init-pop', dest='initial_pop', type=int, default=500, help='Initial population size')
    ...
    parser.add_argument('--output', '--output-dir', dest='output', type=str, default='output/cexo', help='Output directory')
    parser.add_argument('--practical-bulleen', action='store_true', help='Use fixed Bulleen practical-case facility mix')
    parser.add_argument('--sample-bulleen', action='store_true', help='Sample Bulleen practical-case facility counts from configured ranges')
    parser.add_argument('--bulleen-boundary', action='store_true', help='Use approximate irregular Bulleen site boundary')
    parser.add_argument('--bulleen-entrances', action='store_true', help='Use fixed Bulleen practical-case entrance/access points')
    parser.add_argument('--bulleen-roads', action='store_true', help='Add approximate Bulleen road/access corridors as exclusion zones')
    ...
    if args.ablation:
        ...
        if args.sample_bulleen:
            facility_types = sample_practical_bulleen_facility_mix(seed=args.seed)
        elif args.practical_bulleen:
            facility_types = get_practical_bulleen_facility_mix()
        else:
            facility_types = None
        run_mapelites_with_learned_bds(
            ...
            seed=args.seed,
            save_model=not args.test,
            site_width_m=args.site_width_m,
            site_length_m=args.site_length_m,
            export_all_layouts=not args.test,
