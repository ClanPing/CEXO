...
def calculate_operational_efficiency(facilities: List[Dict], entrances: List[Tuple[float, float]]) -> float:
    ...
    if entrances and "office" in facility_positions:
        ...
        sequence_efficiency = total_entrance_access / len(office_positions)
    
    # Updated weighting: Flow(40%) + Access(40%) + Sequence(20%) = 100%
    final_score = 0.4 * flow_efficiency + 0.4 * access_efficiency + 0.2 * sequence_efficiency
