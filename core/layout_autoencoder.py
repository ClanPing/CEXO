...
from .config import FACILITY_SPECS, FACILITY_TYPES
...
def encode_layout_to_vector(facilities: List[Dict], 
    ...
    Encoding scheme:
    - For each facility type: [x, y, presence_flag]
    - Entrance information: [num_entrances, avg_x, avg_y]
    - Global statistics: [centroid_x, centroid_y, avg_spacing]
    ...
    encoding_size = len(FACILITY_TYPES) * 3 + 3 + 3  # facilities + entrances + global
    encoding = np.zeros(encoding_size, dtype=np.float32)
    
    idx = 0
    
    # Encode each facility type
    facility_dict = {f["type"]: f for f in facilities}
    
    for ftype in FACILITY_TYPES:
        if ftype in facility_dict:
            x, y = facility_dict[ftype]["center"]
            encoding[idx] = x
            encoding[idx + 1] = y
            encoding[idx + 2] = 1.0  # Presence flag
        else:
            encoding[idx:idx + 3] = [0.5, 0.5, 0.0]  # Default: center, not present
        idx += 3
    ...
    if len(facilities) > 0:
        ...
        encoding[idx] = centroid[0]
        encoding[idx + 1] = centroid[1]
        ...
        if len(facilities) > 1:
            ...
            encoding[idx + 2] = np.mean(distances)
        else:
            encoding[idx + 2] = 0.0
    else:
        encoding[idx:idx + 3] = [0.5, 0.5, 0.0]
...
def get_encoding_dimension() -> int:
    ...
    return len(FACILITY_TYPES) * 3 + 3 + 3
